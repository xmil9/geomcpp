...
void testWhenBorderIsMuchSmallerThanBoundingBox()
{
   {
      const std::string caseLabel =
         "VoronoiTesselation when border is much smaller than bounding box";

      using Fp = double;

      const Rect<Fp> border{30.0, 30.0, 60.0, 80.0};
      VoronoiTesselation<Fp> vt({{10.0, 10.0},
                                 {100.0, 100.0},
                                 {27.0, 67.0},
                                 {44.0, 23.0},
      ...
      const std::vector<std::vector<Point2<Fp>>> expected = {
         ...
         // Tile 5
         {
            ...
            {60.0, 80.0},
            {60.0, 60.0333333333},
            {57.898123324396785, 56.39008042895442},
            {40.69627507163324, 54.17048710601719},
         },
         // Tile 6
         {
            {30.0, 50.8043478260},
            {34.67028199566, 48.3676789587},
            {33.71847507331, 39.3255131964},
            {30.0, 34.1904761904},
         }};
      VERIFY(hasTiles(tiles, expected), caseLabel);
   }
...
void testForPointsWithDecimals()
{
   {
      const std::string caseLabel = "VoronoiTesselation for points with decimals";

      using Fp = double;

      const Rect<Fp> border{0.0, 0.0, 100.0, 100.0};
      VoronoiTesselation<Fp> vt(
         {
            {10.03982460, 10.874267480},
            {45.3094234, 7.8437662},
            {42.02437654767, 17.02308702},
            {20.00247202, 50.74692212},
         },
         border);
      const std::vector<VoronoiTile<Fp>> tiles = vt.tesselate();
      ...
      const std::vector<std::vector<Point2<Fp>>> expected = {
         // Tile 1
         {
				...
				{100.0, 100.0},
				{100.0, 78.9337298886},
				{23.182534043444, 28.77137746533},
         },
         // Tile 2
         {
				{0.0, 34.56380375503},
				{23.18253404344, 28.77137746},
				{27.4398063005, 6.62616003340},
				{26.8704608261, 0.0},
				{0.0, 0.0},
         },
         // Tile 3
         {
				{100.0, 78.9337298886},
				{100.0, 32.5936189109},
				{27.43980630055, 6.6261600334},
				{23.18253404344, 28.77137746533},
         },
         // Tile 4
         {
				{100.0, 32.5936189109},
...
void testVoronoiTesselation()
{
   ...
   testForMorePoints();
   testForRect();
   testWhenBorderIsMuchSmallerThanBoundingBox();
   testForPointsWithDecimals();
}
